# claude-beginner-harness-plugin

<p align="center"><img src="Thumbnail.png" width="300"></p>
<p align="center"><font size="4"><a href="README.md">English</a> | <strong><a href="README_KR.md">한국어</a></strong></font></p>

Claude Code를 처음 사용하거나 익숙하지 않은 사람들이 **좋은 습관을 만들어가도록 돕는 하네스 플러그인**입니다.

## 설치 방법

```bash
/plugin marketplace add GoldFrosch/claude-beginner-harness-plugin
/plugin install claude-beginner-harness-plugin@beginner-harness
```

[바이브 코딩 가이드](https://drive.google.com/file/d/1x2x1T4lzTISnHGN8nd4KtMhL8iwi6QgU/view)를 참조하면서, 바이브 코딩을 처음 시작할 때 혼자서는 잡기 어려웠던 습관들을 Claude가 자동으로 잡아줄 수 있도록 만든 플러그인입니다.

토큰 사용량이나 작업 시간이 다소 늘어나더라도, 처음부터 올바른 방식으로 Claude와 협업하는 패턴을 익히는 것을 우선합니다.

## 핵심 철학

- **속도보다 습관**: 빠르게 결과를 얻는 것보다, 올바른 과정을 반복해서 몸에 익히는 것이 중요합니다.
- **추측하지 않기**: 불명확한 요구사항으로 작업을 시작하면 방향이 틀린 결과가 나옵니다. 먼저 충분히 이해하고 작업을 시작합니다.
- **초보자 친화적**: Claude Code의 기능을 잘 모르는 사람도 좋은 결과를 낼 수 있도록 안내합니다.

## 작동 방식

이 플러그인은 **3개의 에이전트**가 **11개의 스킬**을 조율하여 작동합니다. 각 에이전트는 특정 역할을 담당하며, 상황에 따라 자동으로 활성화되거나 수동으로 호출할 수 있습니다.

```
코딩 작업 요청
      │
      ▼
[setup-agent] 환경 준비
  ├── init: CLAUDE.md 존재 확인 및 생성
  ├── worktree: 격리 개발 환경 필요성 판단
  └── compile-setup: 빌드 명령어 감지 및 컴파일 hook 등록
      │
      ▼
[design-agent] 작업 설계
  ├── scope: 바이브코딩 vs 협업 설계 판단
  ├── clarify: 요구사항 구체화 (협업 설계 영역)
  ├── impact-check: 영향 분석 및 계약 갱신 (기존 코드 변경 시)
  └── preplan: 복잡한 작업 계획 모드 권유
      │
      ▼
구현 진행
      │
      ▼
[session-agent] 세션 관리 (지속적)
  ├── handoff: 컨텍스트 인계 문서 작성
  ├── capture: 대화 내용 저장
  └── input: INPUT.md 읽기 및 처리

어느 시점에서든 에러 발생 시
      │
      ▼
debug: 근본 원인 분석 → 수정 방향 합의 → debug log 저장
```

## 에이전트

### Setup Agent — 환경 준비 에이전트

작업을 시작하기 전에 프로젝트 환경이 올바르게 준비되어 있는지 점검합니다.

| 항목        | 설명                                                                         |
| ----------- | ---------------------------------------------------------------------------- |
| 역할        | CLAUDE.md 존재 여부 확인, git worktree 격리 필요성 판단, 컴파일 hook 설정   |
| 자동 활성화 | 프로젝트에 CLAUDE.md가 없거나, 대규모 변경이 예상될 때                       |
| 수동 호출   | `/setup`                                                                     |
| 관리 스킬   | `init`, `worktree`, `compile-setup`                                          |

### Design Agent — 작업 설계 에이전트

코딩 작업을 시작하기 전에 올바른 방향을 잡습니다. "일단 만들어보자"는 충동을 막고, 무엇을 어떻게 만들지 먼저 합의합니다.

| 항목        | 설명                                                  |
| ----------- | ----------------------------------------------------- |
| 역할        | 바이브코딩 여부 판단, 요구사항 구체화, 영향 분석 및 계약 갱신, 계획 모드 권유 |
| 자동 활성화 | 모든 코딩 작업 요청 시                                                         |
| 수동 호출   | `/design`                                                                      |
| 관리 스킬   | `scope`, `clarify`, `impact-check`, `preplan`                                  |

### Session Agent — 세션 관리 에이전트

대화가 진행되는 동안 컨텍스트와 작업 흐름이 끊기지 않도록 관리합니다. "대화가 길어지면서 Claude가 앞 내용을 잊어버리는" 문제를 방지합니다.

| 항목        | 설명                                    |
| ----------- | --------------------------------------- |
| 역할        | 컨텍스트 인계, 대화 저장, INPUT.md 처리 |
| 자동 활성화 | 컨텍스트 사용량 50% 이상 추정 시        |
| 수동 호출   | `/handoff`, `/capture`, `/input`        |
| 관리 스킬   | `handoff`, `capture`, `input`           |

## 스킬

### `debug` — 에러 분석 및 학습

에러가 발생했을 때 즉시 수정하는 대신, 근본 원인을 체계적으로 분석하고 왜 이 에러가 났는지 이해하는 습관을 형성합니다. 같은 실수를 반복하지 않도록 수정 후 학습 내용을 저장합니다.

- **자동 활성화**: 사용자가 에러 메시지/스택 트레이스를 공유할 때, 코드 실행 중 에러가 발생했을 때, "왜 안 되지"/"안 돼" 등 예상과 다른 동작 표현 시
- **수동 호출**: `/debug`

**6단계 프로세스**: 에러 분류 → 재현 조건 확인 → 근본 원인 분석(표면 증상 vs 실제 원인) → 수정 방향 합의 → 수정 및 검증 → 재발 방지 요약

수정 후 케이스를 `.claude/debug-log/`에 저장해 이후 참조할 수 있습니다:

```
.claude/debug-log/
  team/       ← git 포함 (프로젝트 공통 패턴)
    INDEX.md
    compile/ / runtime/ / test-failure/ / logic/
  personal/   ← git 제외 (개인 학습 패턴)
    INDEX.md
    compile/ / runtime/ / test-failure/ / logic/
```

Claude가 에러 성격을 보고 **팀** 패턴(프로젝트 특정 모듈/API/비즈니스 로직)인지 **개인** 패턴(언어 기초 실수, 도구, 일반 습관)인지 자동 판단 후 추천하고, 사용자가 최종 확인 후 저장합니다. 파일명은 `YYYY-MM-DD_HH-MM-SS_[에러키워드].md` 형식으로 중복을 방지합니다. 각 scope의 `INDEX.md`에서 케이스를 한 줄 요약으로 탐색할 수 있어 파일을 일일이 열지 않아도 됩니다.

### `compile-setup` — 컴파일 자동 검증 설정

프로젝트의 빌드 명령어를 자동 감지하고 `.claude/settings.json`의 `PostToolUse` hook에 등록해, 파일 수정 후 자동으로 컴파일을 검증합니다.

- **자동 활성화**: setup-agent 실행 시 (프로젝트 초기화 단계)
- **수동 호출**: `/compile-setup`

Node.js/TS, Go, Rust, Java(Maven/Gradle), C#/.NET, Python, C/C++ 등 주요 프로젝트 타입을 자동 감지합니다. 등록 전 사용자에게 명령어를 확인하며, 전체 빌드가 느린 경우(`tsc --noEmit` 등) 빠른 타입 체크 대안을 권유합니다. 기존 hook 설정을 덮어쓰지 않고 병합합니다.

### `init` — CLAUDE.md 초기화

프로젝트 루트에 CLAUDE.md가 없을 때, 어떤 작업보다 먼저 CLAUDE.md 생성을 최우선으로 처리합니다.

- **자동 활성화**: 코딩 작업 요청 시 프로젝트 루트에 CLAUDE.md가 없는 경우
- **수동 호출**: `/init`

CLAUDE.md에는 프로젝트 개요를 간략히 담고, 세부 규칙은 `.claude` 폴더 문서를 참조하도록 작성합니다.

### `scope` — 바이브코딩 범위 판단

코딩 작업 시작 전에 AI가 자율적으로 진행해도 되는 작업인지, 사용자와 협업 설계가 필요한 작업인지 판단합니다.

- **자동 활성화**: 모든 코딩 작업 요청 시
- **수동 호출**: `/scope`

**바이브코딩 영역** (AI 자율 진행): 일회성 페이지, 실험/프로토타입, 데모, 중요도 낮은 독립 코드

**협업 설계 영역** (사용자와 협의): 프로덕션 코드, 보안 관련, 성능 최적화, 핵심 추상화, 데이터 모델 변경

### `clarify` — 요구사항 구체화

작업 요청이 모호할 때 즉시 코드 작업을 시작하는 대신, 요구사항을 먼저 구체화합니다. 협업 설계 영역으로 판단된 작업에만 적용됩니다.

- **자동 활성화**: scope 판단 결과 협업 설계 영역이고, 목적/범위/입출력이 불명확한 경우
- **수동 호출**: `/clarify`

질문지 이전에 **레퍼런스 탐색**을 먼저 실행합니다. AI가 요청 도메인을 파악하고 유명 서비스/게임/도구 중 유사한 것을 선택지로 제시합니다. 레퍼런스가 선택되면 "이것과 다른 점이 있나요?" 한 질문으로 암묵적 계약을 공유하고, 나머지 불명확한 부분만 추가 질문합니다.

**레퍼런스 기반 정렬 방식**: 추상적 설명 대신 공유된 레퍼런스로 정렬하면, 명시하지 않아도 양쪽이 같은 행동 계약을 전제할 수 있습니다.

### `impact-check` — 변경 영향 분석 및 계약 갱신

기존 코드나 요구사항을 변경하기 전에, 영향받는 영역을 파악하고 암묵적 전제를 명시적으로 갱신합니다. 변경 후 문제를 경고하는 방식이 아니라, 변경 전에 각 영향 영역의 이전/이후 계약을 나란히 제시하고 합의한 뒤 진행합니다.

- **자동 활성화**: 기존 코드/요구사항에 대한 변경 요청 시 (인터페이스, 동작, 데이터 모델, 로직 변경)
- **수동 호출**: `/impact-check`
- **clarify 연동**: `/clarify` 결과물이 있으면 해당 Requirements Summary를 계약 기준으로 활용

**계약 갱신형 방식**: 영향받는 각 영역에 대해 "변경 전 전제"와 "변경 후 전제"를 나란히 제시하고, 사용자가 새 계약을 명시적으로 확인한 뒤 코드 작업을 시작합니다. 충돌이 감지되면 범위 확장, 범위 축소, 인지 후 진행 중 방향을 사용자가 결정합니다.

### `preplan` — 작업 시작 전 준비

컨텍스트가 없는 상태에서 복잡한 작업을 시작하기 전, 도메인 용어를 정립하고 필요 시 계획 모드 진입을 권유합니다.

- **자동 활성화**: 초기 컨텍스트 없이 복잡한 작업 요청 시
- **수동 호출**: `/preplan`

계획 모드 권유 조건: 여러 파일/모듈에 걸친 변경, 기존 모듈 연동 신규 기능, 리팩토링, 아키텍처 변경, 복구 비용이 큰 작업

### `input` — INPUT.md 읽기

프로젝트 루트의 `INPUT.md`를 읽고 그 내용에 대해 응답합니다. 대화창에 직접 입력하기 어려운 긴 내용을 파일로 전달할 때 사용합니다.

- **수동 호출**: `/input`

INPUT.md는 버전 관리 시스템 ignore 파일에 자동 등록됩니다.

### `capture` — 대화 내용 저장

마지막 사용자 입력과 Claude의 응답을 `.claude/history/`에 타임스탬프 파일명으로 저장합니다.

- **수동 호출**: `/capture`

### `worktree` — Git Worktree 격리 개발

기능 규모가 크거나 이전 작업과 완전히 다른 작업을 시작할 때 git worktree 생성을 권고합니다.

- **자동 활성화**: 변경 파일이 많은 경우, 핵심 공통 모듈 수정, 이전 작업과 무관한 새 기능 시작 시
- **수동 호출**: `/worktree`
- **MCP 연동**: `EnterWorktree` 도구가 있으면 사용자 승인 후 즉시 worktree 생성

### `handoff` — 컨텍스트 관리 및 인계

대화가 길어져 컨텍스트가 50% 이상 채워졌을 때, 작업 흐름을 끊지 않고 컨텍스트를 초기화할 수 있도록 HANDOFF.md 인계 문서를 작성합니다.

- **자동 활성화**: 컨텍스트 사용량이 50% 이상으로 추정될 때
- **수동 호출**: `/handoff`

## 프로젝트 구조

```
claude-beginner-harness-plugin/
├── .claude-plugin/
│   └── plugin.json          # 플러그인 설정 (v1.1.0)
├── agents/
│   ├── setup/AGENT.md       # 환경 준비 에이전트
│   ├── design/AGENT.md      # 작업 설계 에이전트
│   └── session/AGENT.md     # 세션 관리 에이전트
├── skills/
│   ├── debug/SKILL.md       # 에러 분석 및 학습
│   ├── compile-setup/SKILL.md # 컴파일 자동 검증 설정
│   ├── init/SKILL.md        # CLAUDE.md 초기화
│   ├── scope/SKILL.md       # 바이브코딩 범위 판단
│   ├── clarify/SKILL.md     # 요구사항 구체화
│   ├── impact-check/SKILL.md # 변경 영향 분석 및 계약 갱신
│   ├── preplan/SKILL.md     # 작업 시작 전 준비
│   ├── input/SKILL.md       # INPUT.md 읽기
│   ├── capture/SKILL.md     # 대화 내용 저장
│   ├── worktree/SKILL.md    # Git Worktree 격리 개발
│   └── handoff/SKILL.md     # 컨텍스트 관리 및 인계
├── CLAUDE.md                # 플러그인 지침 문서
└── README.md
```

## 개발 원칙

이 플러그인에 새로운 스킬이나 기능을 추가할 때는 아래 기준을 따릅니다:

1. 초보자가 자주 겪는 실수나 비효율적인 패턴을 해결하는가?
2. 단순히 편의를 제공하는 것이 아니라, 좋은 습관 형성에 기여하는가?
3. 작업 결과보다 작업 과정을 개선하는가?
